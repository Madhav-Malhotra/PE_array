# Makefile provided by Matt Venn and the Zero to ASIC Course Team
...
test_pe:
        rm -rf build/
        mkdir build/
        iverilog -o build/pe.vvp -s pe -s dump -g2012 src/pe.v test/dump_pe.v
        PYTHONOPTIMIZE=${NOASSERT} MODULE=test.test_pe vvp -M $$(cocotb-config --prefix)/cocotb/libs -m libcocotbvpi_icarus build/pe.vvp
...
show_synth_%: src/%.v
        yosys -p "read_verilog $<; flatten; proc; opt; show -colors 2 -width -signed $*"
